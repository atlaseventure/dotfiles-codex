...
- Pi 子代理由 `pi-subagents` 提供。单个子代理通过 `subagent` 工具调用，指定 agent 名称和自包含的 task；可使用 `scout` 做代码检索、`researcher` 做外部资料核验、`worker` 做明确授权的实现、`reviewer` 做代码审查、`oracle` 做高风险决策复核。
...
- 探索型子代理默认不修改源文件；`write` 仅用于任务明确指定的输出文件。需要修改代码时使用 `worker`，并在任务中明确修改范围、验收标准和禁止自行取舍的边界。
